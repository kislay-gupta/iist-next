...
export async function MoreProject({category,projects_slug}:{category:string,projects_slug:string}) {
    const data = await getProducts(category);
    const filteredData = data.filter((item:Project) => item.slug !== projects_slug);
    return (
